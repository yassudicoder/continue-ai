/* Continue AI — shared site motion.
...
(function () {
  ...
  async function heroDemo() {
    ...
    if (reduce) { select(0); if (fromEl) fromEl.textContent = data[0].from; typeEl.textContent = data[0].text; return; }
    ...
    while (true) {
      ...
      query.textContent = "Choose where to continue…"; query.classList.add("ph");
      typeEl.textContent = "";
      await sleep(750);
      query.classList.remove("ph");
      await type(query, "Continue on " + d.to, 60);   // choose a destination
      select(d.idx);                                  // highlight the target
      await sleep(720);
  ...
  /* ============ PEAK: live transfer demo · ⌘K palette · hero tilt ============ */
  /* A short sample conversation for the in-page "Try it" transfer demo. Pure
     client-side simulation — no real extension, no API, no network. */
  var SAMPLE_CHAT = [
    { who: "You",     role: "user", text: "Help me design a rate limiter for our API." },
    { who: "ChatGPT", role: "bot",  text: "A token-bucket is a good fit: each client gets a bucket of N tokens that refills at a steady rate. A request spends a token; empty bucket = rejected." },
    { who: "You",     role: "user", text: "Show it in TypeScript, and handle short bursts gracefully." },
    { who: "ChatGPT", role: "bot",  text: "Here's a TokenBucket with a refill timer and a tryRemove() guard, plus a small burst allowance so quick spikes don't get throttled. (code)" },
    { who: "You",     role: "user", text: "Now make the limit work across 3 servers, not just one process." }
  ];
  ...
  function initTransferDemo(root){
    var thread   = root.querySelector("#tdThread"),
        target   = root.querySelector("#tdTarget"),
        btn      = root.querySelector("#tdTransfer"),
        status   = root.querySelector("#tdStatus"),
        destName = root.querySelector("#tdDestName"),
        destHint = root.querySelector("#tdDestHint"),
        body     = root.querySelector("#tdBody"),
        step     = root.querySelector("#tdStep");
    if (!thread || !btn || !body) return;

    // render the sample conversation
    thread.innerHTML = "";
    SAMPLE_CHAT.forEach(function(m){
      var el = document.createElement("div");
      el.className = "td-msg " + (m.role === "user" ? "user" : "bot");
      el.innerHTML = '<span class="who"></span><span class="tx"></span>';
      el.querySelector(".who").textContent = m.who;
      el.querySelector(".tx").textContent = m.text;
      thread.appendChild(el);
    });
    var msgs = [].slice.call(thread.querySelectorAll(".td-msg"));

    function setStep(t, done){ if (step){ step.textContent = t; step.classList.toggle("done", !!done); } }
    function type(el, txt, sp){
      return new Promise(function(res){ var k = 0;
        (function s(){ if (k <= txt.length){ el.textContent = txt.slice(0, k++); setTimeout(s, sp); } else res(); })(); });
    }
    function buildPrompt(to){
      var lines = ["You are continuing a conversation that was moved over from ChatGPT. Full context below — keep going from where it left off.", "", "── Conversation so far ──"];
      SAMPLE_CHAT.forEach(function(m){ lines.push((m.role === "user" ? "User: " : "Assistant: ") + m.text); });
      lines.push("", "── Continue ──", "Pick up the user's last question: making the rate limit work across 3 servers, not just one process.");
      return lines.join("\n");
    }
    ...
    async function run(){
      ...
      var to = (target && target.value) || "Claude";
      if (destName) destName.textContent = to;
      if (destHint) destHint.textContent = "new chat";
      body.textContent = "";
      if (status) status.textContent = "";

      // 1 — capture the full thread
      setStep("Capturing the full thread…");
      for (var i = 0; i < msgs.length; i++){ msgs[i].classList.add("captured"); await sleep(reduce ? 0 : 130); }
      await sleep(reduce ? 0 : 420);

      // 2 — package into a portable prompt
      setStep("Packaging into a portable prompt…");
    ...
    if (target) target.addEventListener("change", function(){ if (destName) destName.textContent = target.value; });
  }

  function cmdPalette(){
    var CWS = "https://chromewebstore.google.com/detail/continue-ai-%E2%80%94-move-your-a/cojmibmimpeakmgllhpolgbjienedfkf";
    var cmds = [
